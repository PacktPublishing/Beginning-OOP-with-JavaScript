  ...
  isImplementedBy(obj) {
    ...
    if (obj) {
      ...
      for (let i = 0; i < methodsLen; i++) {
        ...
        "function") {
          throw new Error("The object does not implement the interface " + this.name + ". Method " + currentMember +
          " not found.");
        }
      }
      ...
      for (let i = 0; i < propertiesLen; i++) {
        ...
        if (!obj[currentMember] || typeof obj[currentMember] ===
        "function") {
          throw new Error("The object does not implement the interface " + this.name + ". Property " + currentMember
          + " not found.");
        } 
      } 
    } else { 
...
Person.prototype.payWithCreditCard = function(amount) {
  //make a payment
};

Person.prototype.pay = function(amount) {
             return this.payWithCreditCard(amount);
};
...
Developer.prototype.payWithBitcoins = function(amount) {
             //make a payment
};

Developer.prototype.pay = function(amount) {
            return this.payWithBitcoins(amount);
};
